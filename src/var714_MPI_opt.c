...
void relax(float **A, int nrows, int rank, int size, int start_row, float *eps)
{
    ...
    MPI_Irecv(&A[0][0], 3 * N, MPI_FLOAT, prev, 1, MPI_COMM_WORLD, &reqs[nreq++]);
    MPI_Irecv(&A[nrows + 3][0], 3 * N, MPI_FLOAT, next, 2, MPI_COMM_WORLD, &reqs[nreq++]);

    MPI_Isend(&A[3][0], 3 * N, MPI_FLOAT, prev, 2, MPI_COMM_WORLD, &reqs[nreq++]);
    MPI_Isend(&A[nrows][0], 3 * N, MPI_FLOAT, next, 1, MPI_COMM_WORLD, &reqs[nreq++]);
